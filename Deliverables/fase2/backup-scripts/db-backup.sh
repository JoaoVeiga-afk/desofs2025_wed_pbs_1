...
DB_SERVER="server"
LOGS_DB_NAME="logs_database_name"
LOGS_DB_USER="logs_database_user"
LOGS_DB_PASSWORD="logs_database_password"
LOGS_DB_SERVER="logs_server"
...
LOGS_BACKUP_FILE="$BACKUP_DIR/logs_backup_$DATE.sql.gz"
mysqldump -u "$DB_USER" -p"$DB_PASSWORD" -h "$DB_SERVER" "$DB_NAME" | gzip > "$BACKUP_FILE"
mysqldump -u "$LOGS_DB_USER" -p"$LOGS_DB_PASSWORD" -h "$LOGS_DB_SERVER" "$LOGS_DB_NAME" | gzip > "$LOGS_BACKUP_FILE"

# Encrypt the backup file
openssl enc -aes-256-cbc -salt -in "$BACKUP_FILE" -out "$BACKUP_FILE.enc" -k "$ENCRYPTION_PASSWORD"
openssl enc -aes-256-cbc -salt -in "$LOGS_BACKUP_FILE" -out "$LOGS_BACKUP_FILE.enc" -k "$ENCRYPTION_PASSWORD"
...
if [ "$(date +%u)" -eq 7 ]; then
    ...
    cp "$LOGS_BACKUP_FILE.enc" "$BACKUP_DIR/weekly/logs_backup_$DATE.enc"
    ...
    if [[ "$(date +%d)" -le 7 ]]; then
        ...
        cp "$LOGS_BACKUP_FILE.enc" "$BACKUP_DIR/monthly/logs_backup_$DATE.enc"
    fi
    ...
    if [[ "$(date +%j)" -le 7 ]]; then
        ...
        cp "$LOGS_BACKUP_FILE.enc" "$BACKUP_DIR/yearly/logs_backup_$DATE.enc"
    fi
else
    ...
    cp "$LOGS_BACKUP_FILE.enc" "$BACKUP_DIR/daily/logs_backup_$DATE.enc"
fi

## Delete temporary file
rm -f "$BACKUP_FILE.enc"
rm -f "$LOGS_BACKUP_FILE.enc"